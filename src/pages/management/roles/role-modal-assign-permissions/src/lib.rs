...
impl Component for ModalAssignPermissions {
    ...
    fn create(props: Self::Properties, link: ComponentLink<Self>) -> Self {
        ...
        ConsoleService::info(&format!("role permissions = {:?}", props.role_permissions));
        ...
        match msg {
            ...
            Msg::RequestAssignPermissions => {
                ...
                if self.selected_permissions.len() == 0 {
                    ...
                    let request = Request::post(format!("{}/api/v2/roles/{}/permissions", API_URL, self.role_id))
                        .header("access_token", self.access_token.clone())
    ...
    fn change(&mut self, props: Self::Properties) -> ShouldRender {
        if self.role_permissions != props.role_permissions {
...
impl ModalAssignPermissions {
    ...
    fn view_permissions (&self) -> Html {
        ...
        // }
        // ConsoleService::info(&format!("role permissions = {:?}", self.role_permissions));
                    ...
                    >
                        {
                            ...
                            .filter(|(index, permission)| {
                                ...
                                if self.selected_api.is_some() {
                                    for role_permission in self.role_permissions.clone() {
                                        // ConsoleService::info(&format!("permission {:?}", permission.value));
                                        if role_permission.permission_name == permission.value && role_permission.resource_server_name == self.selected_api.clone().unwrap().name {
